...
df = pd.read_csv('./docs/knn/credit_score_classification.csv', low_memory=False)
...
# Label Encode para todas as colunas categóricas (incluindo target)
le_dict = {}
for col in df.select_dtypes(include=["string", "category"]).columns:
    le = LabelEncoder()
    df[col] = le.fit_transform(df[col])
    le_dict[col] = le  # guardar encoder caso precise inverter depois
...
sns.scatterplot(x=X_pca[:, 0], y=X_pca[:, 1], hue=y, style=y, palette="deep", s=60)
plt.xlabel("PCA 1")
plt.ylabel("PCA 2")
